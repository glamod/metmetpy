...
def fill_value(fill_serie, fill_value, self_condition_value = None, fillna = False, 
               out_condition = pd.DataFrame(), out_condition_values = None, 
...
#   - fillna always joins with self and out condition combination
    ...
    msk_na = fill_serie.isna() if fillna else pd.Series(index=fill_serie.index,data=False)
    msk_self = (fill_serie == self_condition_value) if self_condition_value else pd.Series(index=fill_serie.index,data=True)
    ...
    if len(out_condition)>0:
        ...
        if isinstance(condition_dataframe,pd.Series):
            ...
            msk_out = pd.concat(( condition_dataframe[k] == v  for k,v in condition_values.items()),axis = 1).all(axis = 1)
    else:
