...
def imma1(data,conversion):
    ...
    def to_datetime(data):
        ...
        if not empty:
            hours,minutes = np.vectorize(datetime_decimalhour_to_HM)(dt_data.iloc[np.where(not_na)[0],-1].values)
        ...
        # to string   !!!!!
        if not empty:
            dt_series.loc[not_na] = pd.to_datetime(dt_data.loc[not_na,:].astype(int).astype(str).apply("-".join, axis=1).values, format = date_format, errors = 'coerce')
        return dt_series
